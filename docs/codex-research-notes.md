...
### GitHub Actions 런타임 경고는 action 업그레이드로 해소한다

GitHub Actions는 Node.js 20 EOL 이후 JavaScript action 런타임을 Node.js 24로 전환하고 있다. `actions/checkout@v4`는 Node.js 20 기반이라 deprecation 경고가 발생했으므로, Node.js 24 기반으로 릴리즈된 `actions/checkout@v5`로 올렸다.
